...
const TicketEditFormContainer = withStyles(styles)(class extends PureComponent {
  ...
  handleSubmit = (event) => {
    ...
    this.props.editTicket(this.props.match.params.idEvent, this.props.match.params.idTicket, this.state)
    this.setState({
      price: 0,
      description: ''
    })
  ...
  render() {
    const { classes, currentUser, tickets } = this.props
    ...
    return (
      ...
      classes={classes}
      selectedTicket={this.selectTicket(tickets['tickets'])}
      />
    )
  }
})
